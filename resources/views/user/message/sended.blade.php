                ...
                <div class="btn-toolbar p-3" role="toolbar">
                    <div class="btn-group me-2 mb-2 mb-sm-0">
                        ...
                        <div class="dropdown-menu">
                            ...
                            <a class="dropdown-item" href="javascript: void(0)" onclick="filterMessage('seen')">فقط پیام های دیده شده <i class="fa fa-check"></i><i class="fa fa-check"></i></a>
                            <a class="dropdown-item" href="javascript: void(0)" onclick="filterMessage('unseen')">فقط پیام های دیده نشده <i class="fa fa-check"></i></a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <tbody>
                @foreach ($messages as $message)   
                    <tr>
                        ...
                        <td>
                            @if (Redis::zScore('messages',"message:$message->id:read:" . $message->id))
                                <i class="fa fa-check double-check" aria-hidden="true"></i>
                                <i class="fa fa-check double-check" aria-hidden="true"></i>
                            @else
                                <i class="fa fa-check single-check" aria-hidden="true"></i>
                            @endif
                        </td>
                        <td class="text-center align-middle body">{!! Str::limit($message->body,50)  !!}</td>
                        <td class="text-center align-middle">
                        ...
                        <td class="text-center align-middle">
                            <a href="{{ route('message.show',$message->id) }}" class="btn btn-primary" target="_blank"><i class="fa fa-eye"></i></a>
                        </td>
                        <td class="text-center align-middle">
                            <a href="{{ route('message.edit',$message->id) }}" class="btn btn-info" target="_blank"><i class="fa fa-edit"></i></a>
                        </td>
...
@section('footer')
    <script>
        ...
        const filterMessage = (value) => {
            // font awesome icons
            let faCheck = $('.single-check');
            let faDoubleCheck = $('.double-check')
            let oldFilter;
            let newFilter;
         
            // Filter just show seen message and hide unseen message!;
            if(value == 'seen') {
                oldFilter = faDoubleCheck.parent().parent(); 
                newFilter = faCheck.parent().parent(); 
            }
            if(value == 'unseen') {
                oldFilter = faCheck.parent().parent(); 
                newFilter = faDoubleCheck.parent().parent();  
            }
